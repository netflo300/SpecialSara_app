<?php
  ...
  class DB
  {
    ...
    /** Connect to a MySQL database to be able to use the methods below.
      */
    function DB($base=MA_DATABASE, $server=MYSQL_HOTE, $user=MYSQL_UTILISATEUR, $pass=MYSQL_MOT_DE_PASSE)
    {
      $this->mtStart    = $this->getMicroTime();
      $this->nbQueries  = 0;
      $this->lastResult = NULL;
      mysql_connect($server, $user, $pass) or die('Server connexion not possible.'."$server, $user, $pass");
    ...
    function fetch_array($result = NULL)
    {
      ...
      if ($result == NULL || $this->get_num_rows() < 1)
        return NULL;
      else
      {
	$tab = array();      	
